...
async def search_command(message: Message, state: FSMContext):
    ...
    await message.answer("🔍 **Введите код задачи**\n\n" "_Например: /search 2185A_", parse_mode="Markdown")
...
async def top_callback(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            response_text + "\n*Нажмите ⬅️ Назад для меню*", reply_markup=get_back_menu(), parse_mode="Markdown"
        )
    finally:
        db.close()
...
async def filter_menu(callback: CallbackQuery):
    await callback.answer()
    await callback.message.edit_text(
        "🎯 **Фильтры**\n\nВыберите фильтр:", reply_markup=get_filter_menu(), parse_mode="Markdown"
    )
...
async def tags_menu(callback: CallbackQuery):
    await callback.answer()
    await callback.message.edit_text(
        "🎯 **Популярные теги**\nВыберите:", reply_markup=get_tags_menu(), parse_mode="Markdown"
    )
...
async def rating_menu(callback: CallbackQuery):
    await callback.answer()
    await callback.message.edit_text(
        "📊 **Сложность**\nВыберите диапазон:", reply_markup=get_rating_menu(), parse_mode="Markdown"
    )
...
async def tag_filter(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        response_text + "\n*⬅️ Назад*", reply_markup=get_back_menu(), parse_mode="Markdown"
    )
